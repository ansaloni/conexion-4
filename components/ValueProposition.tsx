...
export default function ValueProposition() {
  return (
    <section id="sobre" className="py-24" aria-labelledby="value-title">
      <div className="mx-auto max-w-wrap px-6">
        <div className="mx-auto mb-14 max-w-xl text-center">
          ...
          <h2 id="value-title" className="font-display text-[28px] font-bold text-azul md:text-[38px]">
            Treinamento pensado para quem atende, não para quem decora gramática
          </h2>
          <p className="mt-4 text-[16.5px] text-[#555]">
        ...
        <div className="grid grid-cols-1 gap-7 md:grid-cols-3">
          {items.map((it) => (
            <div
              key={it.title}
              className="rounded-2xl border-t-4 border-azul bg-cinza p-9 transition hover:-translate-y-1.5 hover:shadow-card"
            >
              <div className="mb-5 flex h-13 w-13 items-center justify-center rounded-xl bg-azul text-amarelo" style={{ width: 52, height: 52 }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  {it.icon}
                </svg>
              </div>
              <h3 className="mb-2.5 font-display text-[18.5px] font-bold text-azul">{it.title}</h3>
              <p className="text-[14.5px] text-[#555]">{it.desc}</p>
            </div>
          ))}
        </div>
      </div>
