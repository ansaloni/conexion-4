...
export default function TrustIndicators() {
  return (
    <section id="resultados" className="py-24" aria-labelledby="trust-title">
      <div className="mx-auto max-w-wrap px-6">
        <div className="mx-auto mb-14 max-w-xl text-center">
          <span className="mb-3 block font-display text-xs font-bold uppercase tracking-wider text-vermelho">
            Resultados
          </span>
          <h2 id="trust-title" className="font-display text-[28px] font-bold text-azul md:text-[38px]">
        ...
        <div className="mb-16 grid grid-cols-1 gap-6 md:grid-cols-3">
          {stats.map((s) => (
            <div key={s.label} className="rounded-2xl border border-[#e6e6e6] p-8 text-center">
              <strong className="block font-display text-[38px] font-extrabold text-vermelho">{s.value}</strong>
              <span className="mt-1.5 block text-[13.5px] text-[#666]">{s.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {testimonials.map((t) => (
            <div key={t.name} className="rounded-2xl bg-cinza p-8">
              <p className="mb-6 text-[15px] italic text-grafite">&ldquo;{t.quote}&rdquo;</p>
